...
public class ChainController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        positions = new Vector3[jL];
        total_link_lengths = 0f;

        for (int i=0; i<jL - 1; i++){
            link_lengths[i] = Vector3.Distance(joints[i].position,joints[i+1].position); //determining link lengths by measuring the distance from one joint to another
    ...
    void IniFABRIK(Vector3 Targetposition)
    {
        Debug.Log(Vector3.Distance(endEffector.position, Target.position));
        int n = joints.Length;
        ...
        float dist = Vector3.Distance(positions[0], Targetposition); //distance between root and target
        ...
        else //the target is reachable so set base position as the initial position of the joint p_1
        {
            ...
            for (int iter = 0; iter < iteration; iter++)
            {
                ...
                float dist_to_target = Vector3.Distance(positions[n-1], Targetposition);
                ...
                if (Vector3.Distance(positions[n-1], Targetposition) < tolerance) break;
        ...
        for (int i = 0; i < n - 1; i++)
        {
            ...
            joints[i].rotation = Quaternion.LookRotation(direction) * Quaternion.Euler(90, 0, 0);
        }
